...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *pjvm, void *reserved)
{
    ...
    jmethodID globalMethodIdNetworkSingleton = JNI_Helper::getGlobalMethodIdNetworkSingleton(env);
    if (globalMethodIdNetworkSingleton == nullptr)
    {
        Logger::getInstance()->log("Failed to get globalMethodIdNetworkSingleton method id");
    }
    JNI_Helper::mNetworkExecutorOnSuccessMethodId = globalMethodIdNetworkSingleton;

    jmethodID globalMethodIdMessageHandler = JNI_Helper::getGlobalMethodIdMessageHandler(env);
    if (globalMethodIdMessageHandler == nullptr)
    {
        Logger::getInstance()->log("Failed to get globalMethodIdMessageHandler method id");
    }
...
jmethodID JNI_Helper::getGlobalMethodIdNetworkSingleton(JNIEnv *env){
    ...
    return globalMethodIdNetworkSingleton;
}

jmethodID JNI_Helper::getGlobalMethodIdMessageHandler(JNIEnv *env){
    ...
    return globalMethodIdMessageHandler;
}
